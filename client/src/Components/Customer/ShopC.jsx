...
import { useGetAllFurnitureQuery } from '../../Store/furnitureApiSlice'
...
const ShopC = () => {
    ...
    const filter = () => {
        ...
        if (search) {
            if (from) {
                if (to) {
                    filterFurniture = allFurniture.filter(a => a.price >= from && a.price <= to && a.description.includes(search))
                }
                else {
                    filterFurniture = allFurniture.filter(a => a.price >= from && a.description.includes(search))
                }
            }
            else {
                if (to) {
                    filterFurniture = allFurniture.filter(a => a.price <= to && a.description.includes(search))
                }
                else {

                    filterFurniture = allFurniture.filter(a => a.description.includes(search))
                }
            }
        }
        else {
            if (from) {
                if (to) {
                    filterFurniture = allFurniture.filter(a => a.price >= from && a.price <= to)
                }
                else {
                    filterFurniture = allFurniture.filter(a => a.price >= from)
                }
            }
            else {
                if (to) {
                    filterFurniture = allFurniture.filter(a => a.price <= to)
                }
                else {

                    filterFurniture = allFurniture
                }
            }
        }
        ...
        return (
            <div className="col-12" key={armchair.id}>
                <div className={classNames('flex flex-column xl:flex-row xl:align-items-start p-4 gap-4', { 'border-top-1 surface-border': index !== 0 })}>
                    <img className="w-9 sm:w-16rem xl:w-10rem shadow-2 block xl:block mx-auto border-round" src={`http://localhost:8050/uploads/${armchair?.image?.split("\\")[2]}`} />
            ...
            <div className="col-12 sm:col-6 lg:col-12 xl:col-4 p-2" key={armchair.id}>
                <div className="p-4 border-1 surface-border surface-card border-round">
                    <div className="flex flex-column align-items-center gap-3 py-5">
                        <img className="w-9 shadow-2 border-round" src={`http://localhost:8050/uploads/${armchair?.image?.split("\\")[2]}`} alt={armchair.name} />
    ...
    const listTemplate = (furniture, layout) => {
        return <div className="grid grid-nogutter">{furniture.map((armchair, index) => itemTemplate(armchair, layout, index))}</div>;
    };
