    ...
    useEffect(() => {
        if (data) {
            ...
            data.items.forEach(product => {
                if (product.furniture.stock > product.quantity) {
    ...
    const itemTemplate = (item) => {
        return (
            <div className="flex flex-wrap p-2 align-items-center gap-3">
                <img className="w-4rem shadow-2 flex-shrink-0 border-round" src={`http://localhost:8050/uploads/${item.furniture?.image?.split("\\")[2]}`} />
